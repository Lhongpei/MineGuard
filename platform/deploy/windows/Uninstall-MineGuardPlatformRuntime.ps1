...
if (-not $Principal.IsInRole(
    ...
    throw "The MineGuard Platform uninstaller must run as Administrator."
}
...
function Assert-PlatformReleaseIdentity {
    ...
    foreach ($RequiredPath in @($ManifestPath, $VersionPath)) {
        if (-not (Test-Path -LiteralPath $RequiredPath -PathType Leaf)) {
            throw "Installed Platform identity metadata is incomplete: $RequiredPath"
        }
        ...
        if (($RequiredItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 -or
            ...
            throw "Installed Platform identity metadata is unsafe: $RequiredPath"
        }
    }
    ...
    if ([string]$Manifest.product -ne "MineGuard Platform" -or
        ...
        throw "InstallRoot does not contain an identified MineGuard Platform release."
    }
    ...
    if ($ScriptEntries.Count -ne 1 -or
        ...
        throw "The protected Platform uninstall transaction runner does not match release metadata."
    }
}
...
function Assert-PlatformQuiescent {
    param([string[]]$ProtectedRoots)

    $Service = Get-Service -Name "MineGuardPlatform" -ErrorAction SilentlyContinue
    if ($null -ne $Service -or (Test-Path -LiteralPath (
                "Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\MineGuardPlatform"
            ))) {
        throw "Remove the MineGuardPlatform Windows service before uninstalling the runtime."
    }
    ...
    if ($Running.Count -gt 0) {
        ...
        throw "A process is still running from an immutable Platform directory: $Details"
    }
}

function Remove-PlatformQuarantineTree {
    param([string]$QuarantinePath)

    Assert-OrdinaryDirectoryTree -RootPath $QuarantinePath `
        -Label "Platform uninstall quarantine"
    ...
    if (-not (Test-Path -LiteralPath $MarkerPath -PathType Leaf)) {
        ...
        throw "Refusing a non-empty unmarked Platform uninstall quarantine: $QuarantinePath"
    }
...
function Remove-RecognizedQuarantines {
    ...
    $Prefix = ".mineguard-platform-uninstall-quarantine."
    foreach ($Directory in Get-ChildItem -LiteralPath $RootPath -Directory -Force |
        ...
        if (-not (Test-Path -LiteralPath $MarkerPath -PathType Leaf)) {
            ...
            throw "Refusing an unmarked Platform uninstall quarantine: $($Directory.FullName)"
        }
        $Marker = Get-Content -LiteralPath $MarkerPath -Raw -Encoding UTF8 |
            ConvertFrom-Json
        if ([int]$Marker.schemaVersion -ne 1 -or
            [string]$Marker.product -ne "MineGuard Platform" -or
        ...
        Assert-PlatformQuiescent -ProtectedRoots @($Directory.FullName)
        Remove-PlatformQuarantineTree -QuarantinePath $Directory.FullName
    }
}
...
foreach ($TargetName in $TargetNames) {
    ...
    if (-not $TargetPath.Equals(
        ...
        throw "An immutable Platform target escaped InstallRoot: $TargetPath"
    }
    if (Test-Path -LiteralPath $TargetPath) {
        if (-not (Test-Path -LiteralPath $TargetPath -PathType Container)) {
            throw "An immutable Platform target is not a directory: $TargetPath"
        }
        Assert-OrdinaryDirectoryTree -RootPath $TargetPath `
            -Label "Immutable Platform target"
...
$QuarantineName = ".mineguard-platform-uninstall-quarantine." +
...
try {
    ...
    Assert-PlatformQuiescent -ProtectedRoots @(
    ...
    throw ("Platform uninstall rename failed and rollback was incomplete. " +
...
try {
    Remove-PlatformQuarantineTree -QuarantinePath $QuarantineRoot
}
catch {
    throw ("Platform immutable directories were quarantined, but final deletion failed. " +
