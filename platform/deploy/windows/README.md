...
内网和公网应共用同一个 HTTPS FQDN：公网 DNS 指向公网入口/NAT，内网 DNS 把同一域名解析
到私网入口。接入包中的企业端地址和监管端地址都填写这个稳定域名；当前实例不支持用两个
不同 Host/Origin 分别代表内外网。

1. 初始化口令加密的签发密钥，并把 SPKI-DER SHA-256 登记到独立审批记录；
2. 填写煤矿、企业、比较组和内外网 HTTPS 地址；技术 ID、实例名、版本号以及政府侧
   保管目录由当前版本自动生成。界面只要求选择一个企业交付根目录。底层仍用四个互不包含
   的目录分别保存企业接入包、政府
