...
登录后默认进入“领导简洁模式”。该模式只保留三个入口：

- “今日态势”用四张卡片展示报送覆盖、高优先线索、超期事项和待审批数量，并按行动
  顺序生成三条确定性领导简报；
- “待我决策”只汇总待审批、超期、责任人未明确、P1/P2 事项和红橙安全线索；
- “监管报告”生成当前授权范围内的月度或季度只读报告。

简洁模式中的文字摘要由平台现有报送、趋势、安全和核查数据确定性生成，不调用大
模型，不虚构数字，也不改变事项状态。需要查看原始凭证、算法证据或执行办理时，
点击“查看专业分析依据”切换到“监管专业模式”。模式选择只在当前页面会话内生效，
不把账号、身份或偏好写入浏览器本地存储。
...
- “监管报告”保留为独立工作区，领导与专业人员使用同一确定性报告口径。

页面中的风险分、证据等级和最小上报差额都是技术指标，不是处罚等级或违法认定。
领导应关注覆盖率、重复出现、待复核积压和原始证据是否齐全，而不是只看单一分数。

如需先用较完整的数据体验全部领导功能，可按
[90 天演示数据使用说明](docs/90天演示数据使用说明.md)生成 6 座虚构矿井的
隔离演示库。演示库使用独立 `.mineguard-demo` 状态目录，并在页面持续显示
“演示数据”标识，不得与 `.mineguard` 正式状态目录混用。
...
若要在领导端体验完整时序、重复异常、缺报、来源健康、安全预警和生产核验，
可在独立状态目录生成 90 天六矿演示数据。详见
[90 天多矿演示数据使用说明](docs/90天演示数据使用说明.md)。演示生成器带
目录所有权标记并拒绝写入已有正式状态库。
