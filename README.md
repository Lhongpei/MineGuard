...
Windows 现场首次安装、政府多矿注册、单域名内外网、双层路由端口映射、Caddy HTTPS
和企业 Agent 接入的完整顺序，见
[Windows 现场部署与网络配置手册](docs/mineguard.cn正式上线步骤.md)。
...
用于煤炭领域映射、解释和回复草稿。正式安装只配置一个业务管理员和固定
`api_admin`：业务管理员完成填报、确认和报送；`api_admin` 登录企业端本机页面后
填写模型 API 地址、模型名称和 API Key，不能查看或修改业务数据。连接测试成功后
Key 使用本机保护机制保存，不写进仓库、前端或普通环境文件。政府 Platform 不接触
模型凭据；没有模型时，导入、校验、报送和确定性算法仍可工作。
...
逐矿身份与双 HMAC、业务管理员确认报送、备份恢复和现场验收。
