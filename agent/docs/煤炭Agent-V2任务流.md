...
企业端、矿端采集服务和监管平台仍是三个独立安全域。Agent V2 只读取企业端
SQLite 中已经存在的草稿和已经成功提交的本地历史，不导入监管端源码或数据库，
也不控制矿端设备。
...
## 9. 自动抓数与 edge-agent 的边界

当前 V2 不是网页爬虫、RPA 或设备采集器：

- 不主动登录企业 ERP、MES、皮带秤或煤矿安全系统；
- 不抓取监管平台数据库；
- 不自动创建每日新草稿；
- 不把 edge-agent 遥测复制到企业草稿；
- 不持有来源 HMAC 密钥；
- 不写 PLC、DCS、传感器或其他生产设备。

`edge-agent/` 以只读方式采集矿端遥测，并按独立边缘合同直接送达监管平台。它和
企业填报 Agent 不共享代码、数据库或运行密钥。两者可在业务流程上对齐时间窗口和
来源编号，但不能互相 import。

要实现“数据到齐即体检”，部署单位目前需要提供一个受控上游适配器：

1. 从获准的企业数据源读取或接收数据；
2. 由来源网关形成载荷摘要和来源签名；
3. 通过企业端现有导入接口创建或更新草稿，并保留来源证明；
4. 使用受保护的企业账号会话和 CSRF 调用事件 API；
5. 发送不含秘密、具有稳定幂等编号的 `draft_data_arrived` 事件；
6. V2 只读检查该既有草稿并生成摘要。

仓库当前没有面向机器账号的 Agent V2 服务间认证，也没有上述通用适配器。生产自动
抓数若不适合使用会话 Cookie，应新增独立、最小权限、可轮换凭证和签名契约，不能
把浏览器密码、Cookie 或 CSRF 固化到采集脚本。

正式启用自动抓数前应进行至少一个完整业务周期的影子运行：适配器只读采集、保留来源
签名、导入草稿并触发只读体检，不自动确认或提交；人工照常处理，并并行统计误报、
漏报、数据延迟、来源完整率和人工复核耗时，再决定是否扩大接入范围。
