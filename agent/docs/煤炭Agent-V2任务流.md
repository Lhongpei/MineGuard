...
网页已经支持“上传 CSV，预览映射后生成填报草稿”。CSV 先在企业本地完成编码、分隔符、
日期列和数据类型预检，再按同一表头指纹下已批准的本矿映射、本地规则和可选的受约束
模型建议形成逐列预览。模型只接收表头及脱敏类型统计，不能读取、生成或修改来源数值；
人员只能从固定指标与日报/三班次白名单中选择。确认映射后才写入待复核草稿并记录原件
摘要，上传和映射确认都不会调用报送确认或发送接口。JSON 等既有结构化格式仍沿用严格
导入。该入口要求用户主动提供材料，不会后台登录 ERP/MES；需要自动抓取时使用受控
只读连接器或固定目录。
...
未配置 API 时自动降级为本地识别和人工映射，不影响标准模板导入。环境变量在进程
启动时读取，修改后必须重启服务；API key 不得写入仓库、CSV 或浏览器页面。
