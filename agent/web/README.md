...
1. **数据报送**：页面可为当前矿井设置一个或多个自动发现目录；固定目录或只读连接器发现数据后，只执行格式、字段冲突、非法内容和重复检查，通过后直接进入可靠发送队列。AI 不判断数据是否正常，也不决定是否报送；不能安全解析的内容才进入待人工核验。文件导入和手工填表是兜底入口；
...
主导航中的 **智能助手** 是独立煤炭业务对话区，可用于解释生产数据和政府风险；它不参与自动报送放行。

缺失值显示为空和“未提供”，页面不会用 0、历史均值或模型猜测填补。批次可以是任意
分钟级时间范围、跨自然月、同日多条记录和部分指标。自动来源检查通过后无需逐批人工确认；人工导入
或待核验批次仍由具备权限的企业账号处理。风险回复必须由企业管理员最终确认。

企业页面不提供字段映射设置。导入器在本地白名单规则范围内自动识别，不能安全识别时
保留待核验，不猜测或自动采用歧义列。默认 CSV 模板是“数据时间”加 11 个原子字段的 12 列
生产数据表，数据时间精确到分钟。只读账号的导入、目录配置、扫描和手工填写入口
会直接锁定。
...
- `app.js`：共享登录、智能助手和兼容界面逻辑；当前主导航直接提供智能助手入口。
...
PUT  /api/v2/watch/config  # 保存当前矿井的自动发现目录
